...
class BMapWX {
  ...
  constructor(param) {
    // 确保param是对象
    param = param || {};
    this.ak = param["ak"] || "REqn7Bs2WOh4It2AG4csXUTMkUWEZOue";
  }
  ...
  getWXLocation(type, success, fail, complete) {
    try {
      // 由于移除了定位功能，这里返回一个默认位置
      const defaultLocation = {
        latitude: 27.915056968706804,
        longitude: 120.67756804417415,
        errMsg: "getLocation:ok (default position)"
      };

      if (typeof success === 'function') {
        success(defaultLocation);
      }

      if (typeof complete === 'function') {
        complete(defaultLocation);
      }
    } catch (error) {
      console.error('获取位置信息失败:', error);
      if (typeof fail === 'function') {
        fail({
          errMsg: "getLocation:fail " + (error.message || '未知错误')
        });
      }
    }
  }
